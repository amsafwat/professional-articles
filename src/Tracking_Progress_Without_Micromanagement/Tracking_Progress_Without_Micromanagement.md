...
```mermaid
flowchart LR
    A[Clear goal] --> B[Visible commitment]
    B --> C[Progress evidence]
    C --> D[Regular inspection]
    D --> E{Outside tolerance?}
    E -- No --> F[Team continues]
    E -- Yes --> G[Discuss support or adaptation]
    G --> H[Updated plan or action]
    H --> C
```
